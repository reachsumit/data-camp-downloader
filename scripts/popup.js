...
// Register "Process" button click event
document.getElementById('startProcess').onclick = startProcess;

// Entrypoint: This code gets executed after DOM is ready.
$(document).ready(function(){
	// Hide waiting display (if present)
	$("#waiting").hide();
	// Handle string truncations in smarter way
	...
	chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
	  ...
	  // Call indicate_start function
	  indicate_start(tab_id,tab_url);
	});
});

// Handle display based on current page details
function indicate_start(tab_id,tab_url){
	console.log(tab_id,tab_url);
	
	if ((tab_url.search(/datacamp.com/) != -1)&&((tab_url.search(/courses/) != -1) )){
		// Page is supported. Check events status.
		ask_for_status(tab_id);
		webpage = "datacamp";
	}else{
		// Webpage is not supported
...
// Checkup on current scraping status with background
...
// Background confirmed that scraping has started, so change popup state to busy
...
// All inter-script communication between popup and background
  ...
  function(request, sender, sendResponse) {
	  ...
	  if(request.receiver=="popup"){
			...
			if(request.status=="scraping_done"){
				...
				document.getElementById('pText').innerHTML = "<br>Hello! Please use the process button to parse this page.";
				$("#startProcess").show();
			}
